...
def get_text_from_frames(vid_path: str, start: float, end: float = None, fpm: int = 2):
    ...
    def __get_text(frames_iter: Iterator):
        ...
        for f in frames_iter:
            ...
            # text.append(t)
            text.append((f[0], pt.image_to_string(f[1])))
        return text
...
def get_topics_from_ocr_text(corpus: list[(float, str)]) -> dict:
    # ignoring times
    corpus_list = [
        re.sub(r"\n+", "\n", d[1]).split("\n") for d in corpus if len(d[1]) > 0
    ]
    ...
    all_slides = remove_words_by_length(
        remove_stop_words(
            remove_non_alpha(".".join([" ".join(d) for d in corpus_list]))
        )
    ).lower()
...
def wiki_to_sentences(article: str) -> list:
    ...
    sentences = [
        re.sub("\s+", " ", remove_non_alpha(s)).strip() for s in nltk.sent_tokenize(a)
    ]
...
def clean_wiki_suggestions(sugg: list[str]) -> list[str]:
    ...
    return [
        re.sub("\s+", " ", remove_non_alpha(s).lower().strip())
        for s in sugg
        if not is_banned(s)
    ]
...
def get_topics_from_summaries(summaries: list[str], n_collocs: int = 3):
    ...
    corpus = remove_words_by_length(
        remove_stop_words(" ".join(summaries).strip()), 4
    ).lower()
    collocs = [
        remove_non_alpha(" ".join(c)).lower()
        for c in get_collocations(corpus.split(" "), n_collocs=n_collocs)
    ]
...
def get_wiki_content(
    search_terms: set, page: bool = False, as_list: bool = False
) -> list[str]:
    summaries = []
    for term in search_terms:
...
def sample_corpus(
    topics: set[str], n_samples: int, corpus: list[list[str]] = None, page: bool = False
) -> list[str]:
    if corpus is None:
...
def ancillary_dictionary_dilation(
    topics: dict,
    n_topics: int = 10,
    n_wiki_sugg: int = 3,
    max_tangents: int = 300,
    verbose=True,
):
    ...
    for i in range(1, max_tangents):
        ...
        wiki_sugg = get_wiki_suggestions(
            search_terms=colloc_sugg, n_wiki_sugg=n_wiki_sugg
        )
...
def get_wiki_article(
    search_term: str, page: bool = True, verbose: bool = False
) -> list[str]:
    w_type = "article" if page else "summarie"
    localpath = f"./{w_type}s/{sanitize_filename(search_term)}.txt"
...
def build_scorer(vid_path: str):
    wikipedia.set_rate_limiting(
        rate_limit=True, min_wait=datetime.timedelta(0, 0, 100000)
    )
